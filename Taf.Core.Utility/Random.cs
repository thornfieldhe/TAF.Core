...
namespace Taf.Core.Utility{
    ...
    #region 随机数操作函数
        ...
        static string GenaratePassword(
            PasswordType passwordType =
                PasswordType.Capital | PasswordType.Lowercase | PasswordType.Number | PasswordType.Special
          , int length = 8){
            var str1 = "abcdefghjkmnpqrstuvwxyz";
            var str2 = "ABCDEFGHJKLMNPQRSTUVWXYZ";
            var str3 = "0123456789";
            var str4 = "~!@#$%^&*_+|:?-=;'./";
            var str = "";
            Fx.If((passwordType & PasswordType.Number) == PasswordType.Number).Then(() => str += str3);
            Fx.If((passwordType & PasswordType.Capital) == PasswordType.Capital).Then(() => str += str2);
            Fx.If((passwordType & PasswordType.Lowercase) == PasswordType.Lowercase).Then(() => str += str1);
            Fx.If(passwordType == PasswordType.Special).Then(() => throw new NotSupportedException("密码类型不能全为特殊字符"));
            var target = GetRandomCode(length, str);
            Fx.If((passwordType & PasswordType.Special) == PasswordType.Special).Then(() => {
                var str5   = GetRandomCode(2, str4);
                var index1 = GetRandomInt(0, length);
                var index2 = GetRandomInt(0, length);
                target = target.Remove(index1, 1).Insert(index1, str5[0].ToString())
                               .Remove(index2, 1).Insert(index2, str5[1].ToString());
            });
            return target;
        }
    }
